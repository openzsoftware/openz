...
public class WADGrid extends WADControl {
  ...
  private void generateJSCode() {
    ...
    String str = "function updateHeader(liveGrid, offset) {\n"
        ...
        + "  }\n"
        + "  var vis=0;\n"
        + "  if (liveGrid.visibleRows+currPageStart+offset==currPageStart+liveGrid.metaData.getTotalRows())\n"
        + "     vis =liveGrid.visibleRows-liveGrid.metaData.getNoDataRows();\n"
        + "  else\n"
        + "     vis =liveGrid.visibleRows;\n"
        + "  res = res + ((liveGrid.visibleRows>0)?(currPageStart+offset+1):0) + \" - \""
        + "+ (currPageStart+offset+vis) + \" / \" + (currPageStart+liveGrid.metaData.getTotalRows()-liveGrid.metaData.getNoDataRows());\n"
        + "  if (pageFull) {\n"
    ...
    text.append("      var obj=document.getElementById(newparams[i][2]);\n");
    text.append("      if (obj!=null) {obj.value=newparams[i][1];} \n");
    text.append("    }\n" + "  }\n");
