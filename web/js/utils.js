  ...
  for (var i=0;i<total;i++) {
    if (form.elements[i].type && (form.elements[i].type != "button") && (form.elements[i].type != "submit") && (form.elements[i].type != "image") && (form.elements[i].type != "reset") && (form.elements[i].readonly!="true") && (form.elements[i].name != "Command") && (form.elements[i].name!="") && !form.elements[i].disabled) {
      if (form.elements[i].type.toUpperCase().indexOf("SELECT")!=-1 && form.elements[i].selectedIndex!=-1) {
        script += ((script=="")?"":"&") + form.elements[i].name + "=" + encodeURI(form.elements[i].options[form.elements[i].selectedIndex].value);
      } else if (form.elements[i].type.toUpperCase().indexOf("CHECKBOX")!=-1 || form.elements[i].type.toUpperCase().indexOf("RADIO")!=-1) {
        if (radioValue(form.elements[i]) != null) script += ((script=="")?"":"&") + form.elements[i].name + "=" + encodeURI(radioValue(form.elements[i]));
      } else if (form.elements[i].value!=null && form.elements[i].value!="") {
        script += ((script=="")?"":"&") + form.elements[i].name + "=" + encodeURI(form.elements[i].value);
      }
    }
  }
  return script;
...
function submitForm(field, value, form, bolOneFormSubmission, isCallOut, frameName) {
  ...
  if (document.getElementById('paramBody')) document.getElementById('paramBody').style.cursor = "default";
  return true;
}
...
function submitCommandForm(action, bolValidation, form, newAction, newTarget, bolOneFormSubmission, bolCheckChanges, isCallOut, controlEvt, evt) {
  ...
  if (confirmAction(action)) {
    ...
    if (isPopup == false)
        showcursorwait=true;
    if (newAction != null && newAction.indexOf("../businessUtility/PrinterReports.html") > -1)
        showcursorwait=false;
    if (newAction != null && newAction.indexOf("../ad_actionButton/ActionButton") > -1)
        showcursorwait=false;
    if (newAction == null)
        showcursorwait=false;
    //alert(newAction);
...
function submitCommandFormParameter(action, field, value, bolValidation, form, formAction, newTarget, bolOneFormSubmission, bolCheckChanges, isCallOut, controlEvt, evt) {
  ...
  if (bolCheckChanges==null) bolCheckChanges = false;
  if (isPopup == false) showcursorwait=true;
  if (isCallOut==null) isCallOut = false;
  if (controlEvt==null) controlEvt = false;
  if (controlEvt) {
    if (!evt) evt = window.event;
...
function addUrlParameters(data) {
  ...
  var text = "";
  for (var i=0;i<total;i++) {
    if (data[i][2]=="true") text += ((text!=null && text!="")?"&":"") + data[i][0] + "=" + encodeURI(data[i][1]);
  }
...
function openPDFSession(strPage, strDirectPrinting, strHiddenKey, strHiddenValue, bolCheckChanges) {
  ...
  return submitCommandForm("DEFAULT", false, null, "../businessUtility/PrinterReports.html?inppdfpath=" + encodeURI(strPage) + "&inpdirectprint=" + encodeURI(direct) + "&inphiddenkey=" + encodeURI(strHiddenKey) + ((strHiddenValue!=null)?"&inphiddenvalue=" + encodeURI(strHiddenValue):""), "hiddenFrame", null, bolCheckChanges);
}
...
function openServletNewWindow(Command, bolValidation, url, _name, processId, checkChanges, height, width, resizable, hasStatus, closeControl, hasLoading) {
  ...
  if (Command!=null && Command!="") {
      parameters = addArrayValue(parameters, "Command", Command, false);
  }
  if (_name!=null && Command!=null && (_name=="TREE" | _name=="ATTACHMENT" | _name=="GRIDEXCEL" | _name=="GRIDCSV" | _name=="GRIDPDF" | _name=="LINKS" | 
                                       Command=="PRINT_PDF" | Command=="PRINTPDFCOMP" | Command=="PRINTEXCELCOMP" | Command=="EDIT_HTML" | Command=="EDIT_EXCEL" | Command=="EDIT_PDF" |
                                       Command=="PDF" | Command=="XLS" | Command=="PRINT_HTML")) 
    showcursorwait=false;
  else 
    showcursorwait=true;
  showcursorwait=false;
...
function menuShow(id) {
  if (!top.frameMenu) {
    ...
    var menuWidth = Math.round(350/top.outerWidth*100);
    if (menuWidth > 25) {
       menuWidth = 25;
    }
    if (top.isRTL == true) {
      frameset.cols = "*," + menuWidth + "%,0%";
    } else {
      frameset.cols = "0%," + menuWidth + "%,*";
    }
...
function inputValueForms(name, field) {
  ...
  if (field.type) {
    if (field.type.toUpperCase().indexOf("SELECT")!=-1) {
      if (field.selectedIndex==-1) return "";
        ...
        for (var fieldsCount=0;fieldsCount<length;fieldsCount++) {
          if (field.options[fieldsCount].selected) {
            if (result!="") result += "&";
            result += name + "=" + encodeURI(field.options[fieldsCount].value);
          }
        }
        return result;
      }
      ...
      if (!field.length) {
        if (field.checked) return (name + "=" + encodeURI(field.value));
        else return "";
      } else {
        ...
        for (var i=0;i<total;i++) {
          if (field[i].checked) {
            if (result!="") result += "&";
            result += name + "=" + encodeURI(field[i].value);
          }
        }
        return result;
      }
    } else return name + "=" + encodeURI(field.value);
  }
...
/**
 * Sets Cursor for all Objects (wait or default)
*/
function setCursor(cursoroption){
 var all_obj=new Array();  
 var i=0; 
          // Popup
 /*
          try {
            all_obj=parent.window.opener.document.all;
            //all_obj=parent.frames['frameMenu'].document.all;
            //all_obj=top.frames['frameMenu'].document.all;
            //  all_obj=document.all;
            //if (cursoroption=="default")
            //alert(cursoroption + all_obj.length);
            for(i=0;i<all_obj.length;i++)  {  
            if (cursoroption=="wait")
                all_obj[i].style.cursor="wait";
            else if (cursoroption=="pointer")
                all_obj[i].style.cursor="pointer";
            else 
                all_obj[i].style.cursor="";
            }
          } catch (ignore) {}
          */
          // Frames
          ...
          try {
            all_obj=top.frames['appFrame'].document.all;
            for(i=0;i<all_obj.length;i++)  {  
            if (cursoroption=="wait")
                all_obj[i].style.cursor="wait";
            else if (cursoroption=="pointer")
