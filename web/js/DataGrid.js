...
var intRowHeight=19;
var showgridsumrow ="N";
// The following Vars are used for additional ROWS (SUM-Fuinction)
var columnsum = [];
var columnindex = [];
var columntype = [];
var decimalsep=",";
...
*/
  render: function() {
    ...
    gridContainer.innerHTML = "<div  class='scrolldh' style='float:left;-webkit-user-drag: auto'></div>";
...
*/
  refreshGridDataAfterFilter: function() {
    for (var i = 0; i < columnsum.length; i++) {
        columnsum[i]=0;
    }
 ...
 */
  gridMovePage: function(direction) {
    ...
    if (direction=="PREVIOUSPAGE") {
        showgridsumrow ="N";
        this.metaData.nodataRows = 0;
        //this.liveGrid.metaData.nodataRows = 0;
    }
    this.refreshGridDataAfterFilter();
  ...
  parseStructure: function(rootElem) {
    try {
      ...
      if (status.length>0){
        var type = status[0].getElementsByTagName('type');
        var title = status[0].getElementsByTagName('title');
        var paramtag = status[0].getElementsByTagName('showgridsumrow');
        showgridsumrow = dojox.data.dom.textContent(paramtag[0]);
        paramtag = status[0].getElementsByTagName('locale');
        locale = dojox.data.dom.textContent(paramtag[0]);
        paramtag = status[0].getElementsByTagName('decimalsep');
        decimalsep=  dojox.data.dom.textContent(paramtag[0]);
        paramtag = status[0].getElementsByTagName('thousandsep');
        thousandsep==  dojox.data.dom.textContent(paramtag[0]);
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        // SUM ROW
        var typeName = columns[i].getAttribute('type');
        if (showgridsumrow=="Y" && (typeName=="integer" || typeName=="float")) {
              columnsum.push(0);
              columnindex.push(i);
              columntype.push(typeName);
        }
      }
    ...
    this.updateGrid = function() {
      ...
      if (this.rowNode && (!this.offset || isVisible(this.offset))) {
        for (var i = 0; i < values.length; i++) {
          ...
          if (issum || value == "SUM") {
              issum = true;
              //this.rowNode.cells[i].innerHTML = "<div class=\"DataGrid_Body_Cell_SUM\">" + value + "</div>";
              //this.rowNode.cells[i].innerHTML = "<div class=\"DataGrid_Body_Cell_SUM\" style=\"color: #008000 !important; font-weight: bold;\">" + value + "</div>";
              if (value == "SUM")
                this.rowNode.cells[i].className = this.rowNode.cells[i].className + " DataGrid_Body_Cell_SUMFIRST ";
              else
                this.rowNode.cells[i].className = this.rowNode.cells[i].className + " DataGrid_Body_Cell_SUM ";
          }
          ...
          //console.log(this.rowNode.cells[i].innerHTML + "**");
        }
    ...
    if(trs.length>0){
      ...
      for ( var i=0 ; i < trs.length; i++ ) {
        ...
        for ( var j=0; j < cells.length ; j++ ) {
          ...
          // SUM-ROW
          if (showgridsumrow=="Y" && columnindex[sumindex]==j && ! isNaN(cellContent.split(thousandsep).join("").replace(decimalsep,"."))) {
              columnsum[sumindex]= parseFloat(columnsum[sumindex]) + parseFloat(cellContent.split(thousandsep).join("").replace(decimalsep,".")); 
              dbg++;
              sumindex++;
          }
        }
      ...
      if (showgridsumrow=="Y") {
        this.liveGrid.metaData.nodataRows=1;
        var row = newRows[trs.length] = this.liveGrid.createRowObject(this.lastOffset + trs.length); 
        var cells = trs[trs.length-1].getElementsByTagName("td");
        var values = ["SUM"];
        var sumindex =0;
        for ( var j=0; j < cells.length ; j++ ) {
            var cell = cells[j];
            var cellContent = "";
            if (columnindex[sumindex]==j) {
                if (columntype[sumindex]=="integer")
                    cellContent = new Intl.NumberFormat(locale, { maximumFractionDigits: 0 }).format(columnsum[sumindex]);
                else
                    cellContent = new Intl.NumberFormat(locale, { maximumFractionDigits: 2 , minimumFractionDigits: 2}).format(columnsum[sumindex]);
                //cellContent = columnsum[sumindex];
                sumindex++;
            }
            values.push(cellContent);
        }
