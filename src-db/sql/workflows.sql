...
v_delete_allowed character varying := 'N';
BEGIN

   IF TG_OP = 'DELETE' and c_getconfigoption('gobnodelteindocuments',old.ad_org_id)='Y' THEN
   -- allow deletion of invoices with no lines, that were generated by Bestellwesen || Transaktionen || Lieferantenrechnungen anlegen
   if(TG_TABLE_NAME = 'c_invoice') then
     if(
           -- no lines in current invoice
       (select count(*) = 0 from c_invoiceline where c_invoice_id = OLD.c_invoice_id)
           -- was not altered in any way, i.e. edited manually
       AND (OLD.created = OLD.updated)
           -- was created through this table, where qty was set to 0
       AND (select count(*) > 0 from c_generateinvoicemanual where c_order_id = OLD.c_order_id and qty = 0 and lineamt = 0)
     ) then
       -- deletion allowed
       v_delete_allowed := 'Y';
     end if;
   end if;
   -- allow deletion of inout with no lines, that were generated by Bestellwesen || Transaktionen || Wareneingang anlegen
   if(TG_TABLE_NAME = 'm_inout') then
     if(
           -- no lines in current minout
       (select count(*) = 0 from m_inoutline where m_inout_id = OLD.m_inout_id)
           -- was not altered in any way, i.e. edited manually
       AND (OLD.created = OLD.updated)
           -- was created through this table, where qty was set to 0
       AND (select count(*) > 0 from c_generateminoutmanual where c_order_id = OLD.c_order_id)
     ) then
       -- allow deletion
       v_delete_allowed := 'Y';
     end if;
