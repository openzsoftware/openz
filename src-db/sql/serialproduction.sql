...
BEGIN
  ...
  if new.isautogeneratedplan='Y' and new.c_project_id is null then
    ...
    if (select count(*) from zssm_productionplan_task where c_projecttask_id=new.c_projecttask_id)=0 then
        ...
        v_value:=new.value;
        while (select count(*) from c_project where value=v_value)>0 loop
            v_value:=v_value||'#';        
        end loop;
        insert into c_project(c_project_id,createdby,updatedby,value,name,projectcategory,projectstatus,ad_client_id,ad_org_id,c_currency_id,timeperpiece,setuptime,isautotriggered,isdefault)
               values(v_prj,new.updatedby,new.updatedby,v_value,new.name,'PRP','OR',new.ad_client_id,new.ad_org_id,'102',new.timeperpiece,new.setuptime,new.isautotriggered,'Y');
  ...
  IF c_getconfigoption('prefedineserials',new.ad_org_id)='Y' and (select count(*) from c_project where c_project_id=new.c_project_id and projectcategory='PRO')=1 then
    ...
    if TG_OP = 'INSERT' or (TG_OP = 'UPDATE' and (select count(*) from zsmf_prefedineserials where zssm_workstep_v_id=new.c_projecttask_id and serialnumber like v_value||'-%')>0) then
        ...
        if (select count(*) from m_product where m_product_id=new.m_product_id and new.assembly='Y' and (isserialtracking='Y' or isbatchtracking='Y'))=1 then
            if  (select count(*) from m_product where m_product_id=new.m_product_id and new.assembly='Y' and  isbatchtracking='Y')=1 then -- mengen-Begrenzung bei Chargen
                select coalesce((select to_number(value) from ad_preference where attribute='PEDEFINEDBATCHQTY'),v_qty) into v_maxqty from dual;
                if v_qty>v_maxqty then v_qty:=v_maxqty; end if;
            end if;
            for v_i in 1..v_qty
            LOOP
...
SELECT zsse_dropfunction('zssm_getopendurchreicher');
CREATE OR REPLACE FUNCTION zssm_getopendurchreicher (p_projecttask_id OUT varchar,zssm_workstep_v_id OUT VARCHAR, p_serialnumber OUT varchar,p_hastrx out varchar)
...
BEGIN
  ...
  LOOP
    ...
    if v_ptask is not null then
        ...
        -- Nur schon produzierte...
        for v_cur2 in (select plannedserialnumber from m_internal_consumption l where l.c_projecttask_id=v_ptask and movementtype='P+' and processed='Y')
        LOOP
            p_serialnumber:=v_cur2.plannedserialnumber;
            if (select count(*)  from m_internal_consumption l where l.c_projecttask_id=v_cur.c_projecttask_id and l.plannedserialnumber=v_cur2.plannedserialnumber)>0 then
                p_hastrx:='Y';
            else
                p_hastrx:='N';
            end if;
...
SELECT
    ...
    (select case when count(*)>0 then 'Y' else 'N' end from m_internal_consumption l where l.c_projecttask_id=p.zssm_workstep_v_id and l.plannedserialnumber=p.serialnumber and l.processed='Y') as hastrx 
from Zsmf_Prefedineserials p,c_projecttask t
where t.c_projecttask_id=p.zssm_workstep_v_id and t.iscomplete='N' and t.assembly='Y'
UNION
SELECT 
    o.p_serialnumber as zssm_predefinedserials_v_id,
    o.p_serialnumber as name,
    ...
    o.p_hastrx as hastrx
from zssm_getopendurchreicher() o, c_projecttask p where o.p_projecttask_id=p.c_projecttask_id;
...
BEGIN
  ...
  LOOP
    -- Wird der artikel in diesem P-Plan gleich wieder verbraucht? - Dann ist dieser Plan kein Produktionplan um den Artikel als Output zu produzieren.
    select count(*) into v_count from zspm_projecttaskbom b where  b.m_product_id=p_product and 
           b.c_projecttask_id in (select c_projecttask_id from zssm_productionplan_task_v where zssm_productionplan_v_id=v_cur.zssm_productionplan_v_id and assembly='Y');
...
BEGIN
  --- 1. select all tasks with product AS output
  for v_cur in (select * from zssm_productionplan_task_v where zssm_productionplan_v_id=p_productionplan and assembly='Y')
  LOOP
    -- Wird der artikel in diesem P-Plan gleich wieder verbraucht? - Dann ist dieser Plan kein Produktionplan um den Artikel als Output zu produzieren.
    select count(*) into v_count from zspm_projecttaskbom b where  b.m_product_id=v_cur.m_product_id and 
           b.c_projecttask_id in (select c_projecttask_id from zssm_productionplan_task_v where zssm_productionplan_v_id=v_cur.zssm_productionplan_v_id  and c_projecttask_id!=v_cur.c_projecttask_id);
...
BEGIN
  ...
  END IF;
	...
	*/
	update c_projecttask set qty = p_qty where c_project_id = p_project_id;
EXCEPTION
WHEN OTHERS THEN
...
BEGIN
    ...
    for v_cur in (select c_projecttask_id,taskbegun from c_projecttask where  c_project_id=p_project_id
                  union
                  select c_projecttask_id,taskbegun  from c_projecttask where c_projecttask_id=p_project_id)
    LOOP
        v_task:=v_cur.c_projecttask_id;
        v_begun:=v_cur.taskbegun;
        if v_begun='Y' then
            ...
            */
            for v_cur in (select quantity,qty_available,qty_instock,date_plan,m_product_id from zssm_Workstepbom_V where  zssm_Workstep_V_id=v_task)
            LOOP
                -- Duchreicher, Artikel wird im selben Auftrag produziert
                if (select count(*) from c_projecttask where m_product_id=v_cur.m_product_id and assembly='Y' and c_project_id=p_project_id and c_projecttask_id!=v_task)=0 then 
