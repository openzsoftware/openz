...
CREATE OR REPLACE VIEW c_bpartneremployee_view AS 
            select c_bpartner.C_BPARTNER_ID as c_bpartneremployee_view_id,c_bpartner.C_BPARTNER_ID as C_BPARTNER_ID, AD_CLIENT_ID, AD_ORG_ID, CREATED,ISACTIVE,UPDATED,CREATEDBY, UPDATEDBY, VALUE, NAME, DESCRIPTION, C_BP_GROUP_ID, ISEMPLOYEE, ISSALESREP, REFERENCENO, AD_LANGUAGE,
                   ...
                   isinresourceplan, 'N'::character(1) as isSummary, c_bpartner.ad_image_id, c_bpartner.imageurl,c_bpartner.c_project_id, 0::numeric as holidays,0::numeric as hoursaccount
         ...
         if (select count(*) from zssm_workstep_prp_v where m_product_id=new.m_product_id)=0 and (select count(*) from m_product_org where m_product_id=new.m_product_id and isactive='Y' and isproduction='Y')=0 then
            if new.m_locator_id is not null then
                insert into zssm_workstep_prp_v(zssm_workstep_v_id,  ad_client_id,  
                        ...
                        qty,  issuing_locator,  receiving_locator,  setuptime,  timeperpiece,  isautotriggered,  isautogeneratedplan, startonlywithcompletematerial,  forcematerialscan, producecontinuously, istestingworkstep, copyworkstep)
                values (get_uuid(),new.ad_client_id,  
                    ...
                    1,new.m_locator_id,new.m_locator_id,0,1,'N','Y','N','N','N','N','N');
            ...
            LOOP
                ...
                values (get_uuid(),new.ad_client_id,  
                    ...
                    1,v_cur.m_locator_id,v_cur.m_locator_id,0,1,'N','Y','N','N','N','N','N');
                -- Generate the right BOM
                update m_product_bom set updated=updated where m_product_id=new.m_product_id;
                v_sc:='';
            END LOOP;
         end if;
         update c_projecttask set simplyfiedmanufacturing=new.simplyfiedmanufacturing where m_product_id=new.m_product_id and c_project_id is null;
...
BEGIN
  ...
  -- set default values to orgconfiguration values
  if (TG_OP = 'INSERT') then
      NEW.mfa_active = c_getconfigoption('mfaactivated', NEW.ad_org_id);
      NEW.keeploggedinactivated = c_getconfigoption('keeploggedinactivated', NEW.ad_org_id);
  end if;
...
CREATE OR REPLACE FUNCTION m_product_weight(p_product_id varchar)  RETURNS numeric AS
$BODY$ DECLARE 
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
compliance with the License. You may obtain a copy of the License at http://www.mozilla.org/MPL/MPL-1.1.html
...
v_cur record;
BEGIN
   select weight into v_weight from m_product where m_product_id=p_product_id;
   if v_weight is not null then
        return v_weight;
   end if;
   if (select isbom from m_product where m_product_id=p_product_id)='Y' then
        for v_cur in (select m_productbom_id,bomqty from m_product_bom where m_product_id=p_product_id)
        LOOP
            v_weight2:=coalesce(v_weight2,0) + coalesce(m_product_weight(v_cur.m_productbom_id),0) * v_cur.bomqty;
        END LOOP;
        if coalesce(v_weight2,0)!=0 then 
            v_weight:=v_weight2;
        end if;
   end if;
   return v_weight;
...
SELECT zsse_dropfunction('cDebtPaymentReportingDateFilter');
CREATE OR REPLACE FUNCTION cDebtPaymentReportingDateFilter(p_reportingDate varchar)
  RETURNS SETOF c_debt_payment AS
$BODY$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
compliance with the License. You may obtain a copy of the License at http://www.mozilla.org/MPL/MPL-1.1.html
...
*/
DECLARE
    v_cur public.c_debt_payment;
    v_payment_line public.c_debt_payment;
    v_loop_condition boolean;
    v_settlement_cancel_buf varchar;
BEGIN
    DROP TABLE IF EXISTS c_debt_payment_temp_table;
    CREATE TEMP TABLE c_debt_payment_temp_table (LIKE c_debt_payment);

    FOR v_cur IN (SELECT * FROM c_debt_payment cdp WHERE
                         -- Stichtag liegt nach oder auf dem Buchungsdatum der Rechnung
                         DATE_TRUNC('day', (SELECT dateacct FROM c_invoice WHERE c_invoice_id = cdp.c_invoice_id)) <= TO_DATE(p_reportingDate)
    ) LOOP
        -- erste Zeile der Zahlungen (keine generate id)
        SELECT * INTO v_payment_line FROM c_debt_payment cdp WHERE cdp.c_invoice_id = v_cur.c_invoice_id AND cdp.c_settlement_generate_id IS NULL;

        -- entlang der settlement_cancel und settlement_generates von der ersten Zahlung zur letzten Zahlung hangeln
        v_loop_condition := true;
        WHILE v_loop_condition LOOP
            -- wenn Stichtag vor dem Buchungstag der Zahlung liegt
            IF (TO_DATE(p_reportingDate) < DATE_TRUNC('day', (SELECT dateacct FROM c_settlement WHERE c_settlement_id = v_payment_line.c_settlement_cancel_id))
                -- oder Zahlung noch offen
                OR v_payment_line.c_settlement_cancel_id IS NULL
            ) THEN
                -- und sie noch nicht in der temporären Tabelle liegt
                IF ((SELECT COUNT(*) FROM c_debt_payment_temp_table cdptt WHERE cdptt.c_debt_payment_id = v_payment_line.c_debt_payment_id) < 1) THEN
                    -- füge sie in Tabelle hinzu
                    INSERT INTO c_debt_payment_temp_table VALUES (v_payment_line.*);
                END IF;
                v_loop_condition := false;

            -- oder wenn keine weiteren Zahlungen aus dieser Zahlung hervorgegangen sind
            ELSIF (SELECT COUNT(*) FROM c_debt_payment cdp WHERE cdp.c_settlement_generate_id = v_payment_line.c_settlement_cancel_id) = 0 THEN
                -- dann beende Schleife
                v_loop_condition := false;
            ELSE

                -- ansonsten eine Stufe herabsteigen
                -- die Zahlung auswählen, welche aus der ursprünglichen Zahlung ausgegangen ist

                -- zuerst auf offene Zahlung prüfen
                v_settlement_cancel_buf := v_payment_line.c_settlement_cancel_id;
                SELECT * INTO v_payment_line FROM c_debt_payment cdp WHERE
                    cdp.c_settlement_generate_id = v_settlement_cancel_buf
                    -- Zahlung noch offen
                    AND cdp.c_settlement_cancel_id IS NULL;
                -- ansonsten nehme die Zahlung, mit dem späteren dateacct -> dort wurde Zahlung getätigt und es handelt sich somit nicht um einen Rabatt, etc.
                IF (v_payment_line IS NULL) THEN
                    SELECT * INTO v_payment_line FROM c_debt_payment cdp LEFT JOIN c_settlement cs ON cs.c_settlement_id = cdp.c_settlement_cancel_id
                    WHERE
                    cdp.c_settlement_generate_id = v_settlement_cancel_buf
                    ORDER BY cs.dateacct DESC;
                END IF;
            END IF;
        END LOOP;
    END LOOP;

    RETURN QUERY SELECT * FROM c_debt_payment_temp_table;

    DROP TABLE c_debt_payment_temp_table;
    RETURN;
END;
$BODY$
  LANGUAGE plpgsql VOLATILE
...
SELECT zsse_dropfunction('cDebtPaymentGetIsApproved');
CREATE OR REPLACE FUNCTION cDebtPaymentGetIsApproved(p_cInvoiceID varchar)
  RETURNS varchar AS
$BODY$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
DECLARE
    v_cur RECORD;
BEGIN
    -- return Y, when any paymentline of the invoice is approved
    FOR v_cur IN (SELECT * FROM c_debt_payment WHERE c_invoice_id = p_cInvoiceID) LOOP
        IF (v_cur.isapproved = 'Y') THEN
            RETURN 'Y';
        END IF;
    END LOOP;
...
SELECT zsse_dropfunction('cDebtPaymentGetApprovedBy');
CREATE OR REPLACE FUNCTION cDebtPaymentGetApprovedBy(p_cInvoiceID varchar)
  RETURNS varchar AS
$BODY$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
DECLARE
    v_cur RECORD;
BEGIN
    -- return approvedby, when any paymentline of the invoice is approved
    FOR v_cur IN (SELECT * FROM c_debt_payment WHERE c_invoice_id = p_cInvoiceID) LOOP
        IF (v_cur.isapproved = 'Y') THEN
            RETURN v_cur.approvedby;
        END IF;
    END LOOP;
...
SELECT zsse_dropfunction('cDebtPaymentGetApprovaldate');
CREATE OR REPLACE FUNCTION cDebtPaymentGetApprovaldate(p_cInvoiceID varchar)
  RETURNS varchar AS
$BODY$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
DECLARE
    v_cur RECORD;
BEGIN
    -- return approvaldate, when any paymentline of the invoice is approved
    FOR v_cur IN (SELECT * FROM c_debt_payment WHERE c_invoice_id = p_cInvoiceID) LOOP
        IF (v_cur.isapproved = 'Y') THEN
            RETURN TO_CHAR(v_cur.approvaldate, 'DD.MM.YYYY');
        END IF;
    END LOOP;
