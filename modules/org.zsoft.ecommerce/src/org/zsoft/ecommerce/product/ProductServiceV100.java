...
public class ProductServiceV100 extends WebService {
  private static Logger log4j = Logger.getLogger(ProductServiceV100.class);
     ...
     public Quanities getStockQuantity(String orgId, String username, String password,String mProductId) throws Exception  {
         if (!access(username, password,orgId)) {
           if (log4j.isDebugEnabled())
             log4j.debug("Access denied for user: " + username);
           throw new Exception("Access denied for user: " + username); 
         }
         Quanities qtys=new Quanities();
         SimpleDateFormat sdf= new SimpleDateFormat(sqlDateFormat);
         try {
           ProductData[] data = ProductData.getQuantities(pool, mProductId, orgId);
           if (data.length>0){
             qtys.setCurrentonhandqty(new BigDecimal(data[0].curronhandqty));
             if (!data[0].nextdeliverydate.equals(""))
                  qtys.setNextdeliverydate(sdf.parse(data[0].nextdeliverydate));
             qtys.setNextdeliveryqty(new BigDecimal(data[0].nextdeliveryqty));
           }
         } catch (Exception e) {
           log4j.error(e.getMessage());
           throw new Exception("Exception in Webservice: " + e.getMessage()); 
