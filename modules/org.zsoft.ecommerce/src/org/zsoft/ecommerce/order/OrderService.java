...
public class OrderService  extends WebService {
  private static Logger log4j = Logger.getLogger(OrderService.class);
  
  public OrderResponse submitOrder(String orgId, String username, String password,Order porder)  throws Exception {
    ...
    try {
         orderid = OrderData.InsertOrderHeader(con,pool,orgId,username,porder.getCBpartnerId(),porder.getPaymentrule(),porder.getDeliveryviarule(),porder.getCBpartnerLocationId(),null);
  ...
  public Order getOrder(String orgId, String username, String password,String cOrderId) throws Exception  {
    if (!access(username, password,orgId)) {
      if (log4j.isDebugEnabled())
        log4j.debug("Access denied for user: " + username);
      throw new Exception("Access denied for user: " + username); 
