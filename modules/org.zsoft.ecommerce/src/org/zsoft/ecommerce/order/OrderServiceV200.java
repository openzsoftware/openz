...
public class OrderServiceV200  extends WebService {
  private static Logger log4j = Logger.getLogger(OrderServiceV100.class);
  
  public OrderResponse submitOrder(String orgId, String username, String password,OrderV200 porder)  throws Exception {
    ...
    try {
         ...
         if (!orderid.contains("ERR")){
           OrderlineV200[] orderlines = new OrderlineV200[porder.getOrderlines().length];
           ...
           for (int i = 0; i < orderlines.length; i++) {
             ...
             OrderlineData.InsertAttributes(con,pool, orderlines[i].getMAttributesetinstanceId(), issuccess);
           }
  ...
  public OrderV200 getOrder(String orgId, String username, String password,String cOrderId) throws Exception  {
    if (!access(username, password,orgId)) {
      if (log4j.isDebugEnabled())
        log4j.debug("Access denied for user: " + username);
      throw new Exception("Access denied for user: " + username); 
