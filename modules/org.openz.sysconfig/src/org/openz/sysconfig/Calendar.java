...
public class Calendar  extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
      ...
      try{
        String strcalendar = vars.getStringParameter("inpcalendar");

        if (strcalendar.equals("")) {
        	vars.setSessionValue(getServletInfo() + "|calendar", "353E5C3C74014C8EB3205779F4C2BEE8");
        }
        
        String strback = "/org.openz.sysconfig.ad_forms/Calendar.html";

        //Initializing the Fieldgroups
        String strcalendarfg=""; //Navigation Fieldgroup (Barcode Field and Buttons)
        
        String strmsg = Utility.messageBD(this, "SC_calendar",vars.getLanguage());
        
        String strokcalendar="";
        
        //Initializing the Template Structure
        String strSkeleton=""; //Structure
        String strOutput ="" ; //The html-code as Output

        vars.setSessionValue("calendar", strcalendar);

        if (vars.commandIn("NEXT")){
            response.sendRedirect(strDireccion + "/org.openz.sysconfig.ad_forms/Acctschema.html");
            vars.setSessionValue(getServletInfo() + "|calendar", strcalendar);
            
            if(strcalendar.equals("")) {
            	strokcalendar = "<p style=\"color: red;\">X</p>";
            }else {
            	strokcalendar = "<p style=\"color: green;\">OK</p>";
            }
            
            vars.setSessionValue("okcalendar", strokcalendar);
            vars.setSessionValue(getServletInfo() + "|okcalendar", strokcalendar);
        }
        ...
        String strbreadcrump = "<table style=\"margin: 0% 10px -9% 0%;text-align: left;float: right;color: #606060;font-size: 9pt;\">"
        		...
        		+ "<tr style=\"color: black\"><td>→</td><td>Buchungsperioden</td></tr>"
        ...
        strSkeleton = ConfigureFrameWindow.doConfigure(this,vars,"inpcalendar",null, "Initial Calendar",strToolbar,"NONE",tabs);
        ...
        strcalendarfg=fh.prepareFieldgroup(this, vars, script, "SC_calendar_fg", null,false);
        
        //Generating the html output - Replacing the @Content@ with called areas in this case INFOBAR, NAVIGATION, STATUS
        strOutput=Replace.replace(strSkeleton, "@CONTENT@",strbreadcrump + strcalendarfg + strOwnBtn + strinfo);
        
        // Enable Shortcuts
        script.addHiddenShortcut("linkButtonSave_New");
        script.enableshortcuts("EDITION");
        
        script.addJScript("function funcCalendar(){"
        		...
        		+ "document.getElementById(\"calendar\").style.height = \"26px\";"
        		+ "document.getElementById(\"calendar\").style.fontSize = \"15pt\";"
