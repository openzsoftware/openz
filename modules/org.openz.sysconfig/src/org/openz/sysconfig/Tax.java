...
public class Tax  extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
      ...
      try{
        String strtax = vars.getStringParameter("inptax");
        String strtaxincluded = vars.getStringParameter("inptaxincluded");
        
        String strchecktax = vars.getSessionValue("tax");
        
        if (strchecktax.equals("")) {
        	vars.setSessionValue(getServletInfo() + "|tax", "751E587B82E74FD18E8669D3E966D145");
        }
        
        String strback = "/org.openz.sysconfig.ad_forms/Tax.html";

        //Initializing the Fieldgroups
        String strtaxfg=""; //Navigation Fieldgroup (Barcode Field and Buttons)
        
        String strmsg = Utility.messageBD(this, "SC_tax",vars.getLanguage());
        ...
        if (strtaxincluded.equals("")) {
        	strtaxincluded = "N";
        }
        
        vars.setSessionValue("tax", strtax);
        vars.setSessionValue("taxincluded", strtaxincluded);

        if (vars.commandIn("NEXT")){
        	response.sendRedirect(strDireccion + "/org.openz.sysconfig.ad_forms/SysConfigOutput.html");
            vars.setSessionValue(getServletInfo() + "|tax", strtax);
            vars.setSessionValue(getServletInfo() + "|taxincluded", strtaxincluded);
            
            if(strtax.equals("")) {
            	stroktax = "<p style=\"color: red;\">X</p>";
            }else {
            	stroktax = "<p style=\"color: green;\">OK</p>";
            }
            
            vars.setSessionValue("oktax", stroktax);
            vars.setSessionValue(getServletInfo() + "|oktax", stroktax);
        }
        ...
        String strbreadcrump = "<table style=\"margin: 0% 10px -9% 0%;text-align: left;float: right;color: #606060;font-size: 9pt;\">"
        		...
        		+ "<tr style=\"color: black\"><td>→</td><td>Steuer</td></tr>"
        ...
        strSkeleton = ConfigureFrameWindow.doConfigure(this,vars,"inptax",null, "Initial Tax",strToolbar,"NONE",tabs);
        ...
        strtaxfg=fh.prepareFieldgroup(this, vars, script, "SC_tax_fg", null,false);
       
        //Generating the html output - Replacing the @Content@ with called areas in this case INFOBAR, NAVIGATION, STATUS
        strOutput=Replace.replace(strSkeleton, "@CONTENT@",/*strPdcInfobar+*/strbreadcrump + strtaxfg + /*strButtonsFG*/ strOwnBtn + strinfo/*strStatusFG*/);
        
        // Enable Shortcuts
        script.addHiddenShortcut("linkButtonSave_New");
        script.enableshortcuts("EDITION");
        
        script.addJScript("function funcTax(){"
        		...
        		+ "document.getElementById(\"tax\").style.height = \"26px\";"
        		+ "document.getElementById(\"tax\").style.fontSize = \"15pt\";"
        		+ "document.getElementById(\"taxincluded\").style.marginTop = \"7px\";"
