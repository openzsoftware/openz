...
public class Email  extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
      ...
      try{
        ...
        String stremailfg=""; //Navigation Fieldgroup

        String strmsg = Utility.messageBD(this, "SC_email",vars.getLanguage());
        
        String strokemail = "";
        
        String strback = "/org.openz.sysconfig.ad_forms/Email.html";
        
        //Initializing the Template Structure
        String strSkeleton=""; //Structure
        String strOutput ="" ; //The html-code as Output

        vars.setSessionValue("email", stremail);
        
        if (vars.commandIn("NEXT")){
            response.sendRedirect(strDireccion + "/org.openz.sysconfig.ad_forms/Language.html");
            vars.setSessionValue(getServletInfo() + "|email", stremail);
            
            if(stremail.equals("")) {
            	strokemail = "<p style=\"color: red;\">X</p>";
            }else {
            	strokemail = "<p style=\"color: green;\">OK</p>";
            }
            
            vars.setSessionValue("okemail", strokemail);
            vars.setSessionValue(getServletInfo() + "|okemail", strokemail);
        }

        if (vars.commandIn("LAST")){
        	response.sendRedirect(strDireccion + "/org.openz.sysconfig.ad_forms/Name.html");
        	vars.setSessionValue(getServletInfo() + "|email", stremail);
        ...
        String strbreadcrump = "<table style=\"margin: 0% 10px -9% 0%;text-align: left;float: right;color: #606060;font-size: 9pt;\">"
        		...
        		+ "<tr style=\"color: black\"><td>→</td><td>Email</td></tr>"
        ...
        strSkeleton = ConfigureFrameWindow.doConfigure(this,vars,"inpemail",null, "Initial Email",strToolbar,"NONE",tabs);
        ...
        stremailfg=fh.prepareFieldgroup(this, vars, script, "SC_email_fg", null,false);

        //Generating the html output - Replacing the @Content@ with called areas in this case INFOBAR, NAVIGATION, STATUS
        strOutput=Replace.replace(strSkeleton, "@CONTENT@",strbreadcrump + stremailfg + strOwnBtn + strinfo);
        
        // Enable Shortcuts
        script.addHiddenShortcut("linkButtonSave_New");
        script.enableshortcuts("EDITION");

        script.addJScript("function funcEmail(){"
        		...
        		+ "document.getElementById(\"email\").style.height = \"26px\";"
        		+ "document.getElementById(\"email\").style.fontSize = \"15pt\";"
