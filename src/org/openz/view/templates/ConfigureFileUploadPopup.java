...
public class ConfigureFileUploadPopup{
 
  
	  public static String doConfigure(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script, String title, String relabel, String recIdentifier,String key, String servletname,String command) throws Exception{
		    String retval="";
		    ...
		    Object template =  servlet.getServletContext().getAttribute("FileUploadPopupTEMPLATE");
		    if (template==null) {
		      template = new String(FileUtils.readFile("FileUploadPopup.xml", directory + "/src-loc/design/org/openz/view/templates/"));
		      servlet.getServletContext().setAttribute("FileUploadPopupTEMPLATE", template);
		    }
		   ...
		   // retval=Replace.replace(retval, "@ADDITIONALSCRIPTS@", additionalscripts );
		    retval=Replace.replace(retval, "@RECLABEL@", relabel);
		    retval=Replace.replace(retval, "@RECID@", recIdentifier);
		    retval=Replace.replace(retval, "@KEY@",key);
		    retval=Replace.replace(retval, "@SERVLET@", servletname);
		    retval=Replace.replace(retval, "@COMMAND@", command);
		    retval=Replace.replace(retval, "@MULTIPLE@", "");
