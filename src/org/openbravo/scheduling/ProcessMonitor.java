...
class ProcessMonitor implements SchedulerListener, JobListener, TriggerListener {
  ...
  public synchronized void jobScheduled(Trigger trigger) {
  ...
  public synchronized void triggerFired(Trigger trigger, JobExecutionContext jec) {
  ...
  public synchronized void jobToBeExecuted(JobExecutionContext jec) {
    ...
    try {
      if (ProcessRunData.isExecuting(getConnection(), jec.getJobDetail().getName()).equals("N")) {
          ...
          jec.getScheduler().getContext().put(jec.getJobDetail().getName(),"X");
  ...
  public synchronized void jobWasExecuted(JobExecutionContext jec, JobExecutionException jee) {
    final ProcessBundle bundle = (ProcessBundle) jec.getMergedJobDataMap().get(ProcessBundle.KEY);
    if (bundle == null) {
      return;
    }
    ...
    try {
     ...
     // jec.getScheduler().getContext().put(jec.getJobDetail().getName(),"O");
    } catch (final Exception e) {
      log.error(e.getMessage(), e);
    }
  }
  ...
  public synchronized void triggerFinalized(Trigger trigger) {
    try {
  ...
  public synchronized void jobUnscheduled(String triggerName, String triggerGroup) {
    try {
  ...
  public void triggerMisfired(Trigger trigger) {
    /* Not Implemented - leads to dead JOB-Queues */ 
 //   try {
      //ProcessRequestData.update(getConnection(), MISFIRED, trigger.getName());
      String test=trigger.getName();
      log.error("Process Trigger Misfires. ID:" + test);
  ...
  public synchronized boolean vetoJobExecution(Trigger trigger, JobExecutionContext jec) {
    try {
      // SZ: We suppress Parallel Running the same Process. This avoids deadlocks.
      //Thread.sleep(200);
     // String test=jec.getJobDetail().getName();
     // 7C674672BB9A4ECBB6E12023D0FBA425
     // if (test.equals("BBE203799EC14A3AB0BCB519B60AD316"))
     // test="";
      if (ProcessRunData.isExecuting(getConnection(), jec.getJobDetail().getName()).equals("Y")) {
        return true;
      } else {
        return false;
      }
