...
public class ToolBar {
  ...
  private String getButtonScript(String name) {
    if (name.equals("RELATED_INFO")) {
      ...
      return ((grid_id == null || grid_id.equals("")) ? ""
          ...
          + "+'&inpEditable="
          + (isEditable ? "Y" : "N")
          + "', 'ATTACHMENT', null, true, 600, 1000, true);";
  ...
  public void prepareRelationTemplate(boolean hasTree, boolean isFiltered, boolean isTest,
    ...
    //if (!email) 
    //  removeElement("EMAIL");
    if (pdf != null && !pdf.contains("orders") && !pdf.contains("invoices") && !pdf.contains("shipments")  && !pdf.contains("subscription")  &&!pdf.contains("dunrun") ) {
      removeElement("EMAIL");
    }
  ...
  public String toString() {
    ...
    try {
      ...
      if (!isNew && UtilsData.getOrgConfigOption(conn, "toolbardragndrop","0").equals("Y")) {
        ...
        toolbar.append("<table class=\"Main_ContentPane_ToolBar Main_ToolBar_bg\" ondragenter=\"drop(openServletNewWindow('NEW', false, '../businessUtility/TabAttachments_FS.html?inpKey='+" +keyfield+".value+'&amp;inpEditable="+ (isEditable ? "Y" : "N") +"', 'ATTACHMENT', null, true, 600, 1000, true))\" ondrop=\"allowDrop(event)\" id=\"tdToolBar\">\n");}
        else
          toolbar.append("<table class=\"Main_ContentPane_ToolBar Main_ToolBar_bg\" ondragenter=\"drop(openServletNewWindow('NEW', false, '../businessUtility/TabAttachments_FS.html?inpKey=' + dijit.byId('grid').getSelectedRows()+'&amp;inpEditable="+ (isEditable ? "Y" : "N") +"', 'ATTACHMENT', null, true, 600, 1000, true))\" ondrop=\"allowDrop(event)\" id=\"tdToolBar\">\n");
      } else
