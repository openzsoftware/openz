...
public class SessionExpirationFilter implements Filter {
  ...
  public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
    ...
    if (null != session) {
      ...
      String ExpirationTime = (String) session.getAttribute("#SESSIONEXPIRATION");
      Date expirationDate = (Date) session.getAttribute("expirationDate");
      if (ExpirationTime==null){
        ExpirationTime="30";
      }
      Integer SessionDuration=Integer.parseInt(ExpirationTime)*60000;
      if (SessionDuration==0){
        SessionDuration=30*60000;
      }
      if (expirationDate == null)
      
          expirationDate = new Date(System.currentTimeMillis() + 1); // only
